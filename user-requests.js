...
function populateMonthYearFilters(containerId, requests) {
  ...
  // First option is "All Months" with value -1
  monthSelect.innerHTML = '<option value="-1" selected>All Months</option>' +
    months
      .sort((a, b) => a - b)
      .map(m => `<option value="${m}">${new Date(0, m).toLocaleString('en-US', { month: 'long' })}</option>`)
      .join('');
  
  // First option is "All Years" with value -1
  yearSelect.innerHTML = '<option value="-1" selected>All Years</option>' +
    years
      .sort((a, b) => a - b)
...
function filterRequests(containerId) {
  ...
  items.forEach(item => {
    ...
    // Logic for filtering:
    // If both month and year are -1, show all items
    // If only month is -1, filter by year only
    // If only year is -1, filter by month only
    // If neither is -1, filter by both month and year
    const matchesMonth = month === -1 || date.getMonth() === month;
    const matchesYear = year === -1 || date.getFullYear() === year;
